...
class DataPipeline:
    ...
    def load_raw_data(self) -> pd.DataFrame:
        ...
        logger.info(f"Date range: {df[self.model_config['date_column']].min()} to {df[self.model_config['date_column']].max()}")
    ...
    def create_forecasting_dataset(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        agg_df = df.groupby([
            ...
            self.model_config['target_column']: 'sum',
            'AMOUNT': 'sum',
            'RATE': 'mean',
            'BOX': 'sum'
        }).reset_index()
    ...
    def create_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Sort by date
        df = df.sort_values([self.model_config['date_column'], self.model_config['center_column'], self.model_config['item_column']])
        ...
        logger.info(f"Feature columns: {[col for col in df.columns if col not in [self.model_config['date_column'], self.model_config['center_column'], self.model_config['item_column'], self.model_config['target_column']]]}")
    ...
    def _create_lag_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df = df.sort_values([self.model_config['center_column'], self.model_config['item_column'], self.model_config['date_column']])
        
        for lag in self.feature_config['lag_features']:
            df[f'lag_{lag}'] = df.groupby([
                self.model_config['center_column'], 
                self.model_config['item_column']
            ])[self.model_config['target_column']].shift(lag)
    ...
    def _create_rolling_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for window in self.feature_config['rolling_windows']:
            df[f'rolling_mean_{window}'] = df.groupby([
                self.model_config['center_column'], 
                self.model_config['item_column']
            ])[self.model_config['target_column']].transform(
