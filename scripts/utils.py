...
class ForecastEngine:
    ...
    def generate_forecast(self, centers: List[str], items: List[str], 
                        forecast_days: int = 30, model_type: str = "xgboost") -> Tuple[Dict, Dict, Optional[Dict]]:
    ...
    def _create_future_dataframe(self, historical: pd.DataFrame, future_dates: pd.DatetimeIndex,
                               center: str, item: str) -> pd.DataFrame:
        """Create future dataframe with features"""
    ...
    def get_inventory_recommendations(self, center: str, item: str, 
                                    current_stock: float, lead_time: int = 1) -> Dict:
        ...
        try:
            ...
            if center in forecasts and item in forecasts[center]:
                ...
                stock_out_risk = max(0, (lead_time_demand - current_stock) / lead_time_demand * 100) if lead_time_demand > 0 else 0
